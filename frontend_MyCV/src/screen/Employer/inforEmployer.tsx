...
const InforEmployer = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://192.168.58.88:3000/employers');
    ...
    const handleSave = async () => {
        if (formData) {
            try {
                const response = await axios.patch(`http://192.168.58.88:3000/employers/${formData.id}`, formData);
    ...
    const handleDelete = async (id: string) => {
        try {
            await axios.delete(`http://192.168.58.88:3000/employers/${id}`);
